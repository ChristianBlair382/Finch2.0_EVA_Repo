...
class Room_Map:
    ...
    def draw_map(self): 
        # draws the map onto an empty canvas using the anchors and lines 
        plt.figure(figsize=(10, 10))
        plt.xlim(self.room_min_x - 100, self.room_max_x + 100)
        plt.ylim(self.room_min_y - 100, self.room_max_y + 100)
        plt.scatter(self.finch_x, self.finch_y, color='red', label='Finch')
        for anchor in self.anchorList:
            plt.scatter(anchor[0], anchor[1], color='blue', label='Anchor')